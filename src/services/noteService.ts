  ...
  if (settings.saveLocation) {
    ...
    try {
      ...
      try {
        ...
        // If the title changed and the save was successful, update the note ID to match the new filename
        // This is crucial for subsequent renames to work correctly
        if (titleChanged && result.success) {
          // Create the new safe title that will be used as the filename
          const newSafeTitle = finalNote.title.trim().replace(/[^a-z0-9]/gi, '_').toLowerCase();

          // Update the note ID to match the new filename
          // This ensures that future renames will find the correct file
          finalNote.id = newSafeTitle;

          console.log('Updated note ID after rename:', {
            oldId: updatedNote.id,
            newId: finalNote.id,
            newTitle: finalNote.title
          });
        }
      } catch (saveError) {
